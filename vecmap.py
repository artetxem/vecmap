...
class VecMap:
    ...
    def _build_unsupervised_seed_dictionary(self, x_words, x, z_words, z):
        size = min(x.shape[0], z.shape[0], self._unsupervised_dictionary_size)
        u, s, vt = self.xp.linalg.svd(x[:size], full_matrices=False)
        xsim = (u*s).dot(u.T)
        u, s, vt = self.xp.linalg.svd(z[:size], full_matrices=False)
        zsim = (u*s).dot(u.T)
        del u, s, vt
        xsim.sort(axis=1)
        zsim.sort(axis=1)
    ...
    def _build_identical_seed_dictionary(self, x_words, x, z_words, z):
    ...
    def _build_numerals_seed_dictionary(self, x_words, x, z_words, z):
        numeral_regex = re.compile('^[0-9]+$')
        x_numerals = {word for word in x_words if numeral_regex.match(word) is not None}
        z_numerals = {word for word in z_words if numeral_regex.match(word) is not None}
        return self._build_identical_seed_dictionary(x_numerals, x, z_numerals, z)
    ...
    def _read_dictionary(self, path):
        ...
        with open(path, encoding=self.encoding, errors='surrogateescape') as f:
            for line in f:
                ...
                try:
                    xi.append(self.src_word2ind[x_word])
                    zi.append(self.trg_word2ind[z_word])
                except KeyError:
                    oov.append((x_word, z_word))
        return xi, zi, oov


    def _build_seed_dictionary(self, x_words, x, z_words, z, seed_dictionary_path):
        if self._init_dictionary_mode=='unsupervised':
            return self._build_unsupervised_seed_dictionary(x_words, x, z_words, z)
        if self._init_dictionary_mode=='numerals':
            return self._build_numerals_seed_dictionary(x_words, x, z_words, z)
        if self._init_dictionary_mode=='identical':
            return self._build_identical_seed_dictionary(x_words, x, z_words, z)
    ...
    def _select_indices_for_dictionary(self, x, x_size, z, z_size, keep_prob, batch_size):
        ...
        indices = self.xp.zeros(z_size, dtype=int)
        for b in range(0, x_size, batch_size):
            sim = x[b:b+batch_size].dot(z[:z_size].T)
            best_sim[b:b+batch_size] = sim.max(axis=1)
            sim -= knn_sim[b:b+batch_size]/2
            dropout(sim, 1 - keep_prob).argmax(axis=1, out=indices[b:batch_size])
    ...
    def update_dictionary(self, x_words, x, z_words, z, keep_prob, seed_dictionary_path, batch_size):
        if self.dictionary is None:
            self.dictionary = self._build_seed_dictionary(x_words, x, z_words, z, seed_dictionary_path)
            objective = -100
        else:
            self.dictionary, objective = self._rebuild_dictionary(x, z, keep_prob, batch_size)
        return self.dictionary, objective
    

    def set_validation_dictionary(self, path):
        xi, zi, oov = self._read_dictionary(path)
        self.validation_set = {x: z for x, z in zip(xi, zi)}
        validation_coverage = len(self.validation_set) / (len(self.validation_set) + len(oov))
        print(f"Validation set coverage: {validation_coverage}")
    ...
    def validate(self, x, z):
        ...
        accuracy = np.mean([1 if nn[i] in self.validation_set[src[i]] else 0 for i in range(len(src))])
    ...
    def _read_inputs(self, src_input, trg_input):

        with open(src_input, encoding=self.encoding, errors='surrogateescape') as srcfile:
            x_words, x = embeddings.read(srcfile, dtype=self.dtype)
        with open(trg_input, encoding=self.encoding, errors='surrogateescape') as trgfile:
            z_words, z = embeddings.read(trgfile, dtype=self.dtype)
        x = self.xp.asarray(x)
        z = self.xp.asarray(z)
        embeddings.normalize(x, self.normalization_actions)
        embeddings.normalize(z, self.normalization_actions)

        self.src_word2ind = {word: i for i, word in enumerate(x_words)}
        self.trg_word2ind = {word: i for i, word in enumerate(z_words)}
    ...
    def train(
        ...
        x, x_words, z, z_words = self._read_inputs(src_input, trg_input)
        ix, iz = self._build_seed_dictionary(x_words, x, z_words, z, seed_dictionary)
        self.xw, self.zw = self._map(x, ix, z, iz, True)
    ...
    def self_learning_train(
        ...
        while not finish:
            ...
            (ix, iz), objective = self.update_dictionary(x_words, xw, z_words, zw, keep_prob, seed_dictionary, dict_update_batch_size)
            ...
            t0 = time.time()
            xw, zw = self._map(x, ix, z, iz, finish)
